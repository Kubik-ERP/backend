...
import { IsArray, IsNotEmpty, IsOptional, IsString, ValidateNested, IsUUID, IsInt, Min } from 'class-validator';
...
export class CreateTransferStockDto {
    ...
    @ApiProperty({ example: 'Transfer stok barang kebutuhan cabang Bogor', required: false })
    @IsOptional()
    @IsString()
    note?: string;
