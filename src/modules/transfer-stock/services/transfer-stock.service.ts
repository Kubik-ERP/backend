...
export class TransferStockService {
    ...
    async findAllTransferStock(dto: TransferStockListDto, header: ICustomRequestHeaders) {
    ...
    async findAllRequestStock(dto: TransferStockListDto, header: ICustomRequestHeaders) {
        const store_id = requireStoreId(header);
        const orderByField = camelToSnake(dto.orderBy);
        const orderDirection = dto.orderDirection;
    ...
    async findAllItem(header: ICustomRequestHeaders, dto: ItemListDto, search?: string) {
        ...
        const sourceSkus = sourceItems.map((item) => item.sku).filter((sku): sku is string => !!sku);
        if (sourceSkus.length === 0) {
            return {
                items: [],
                meta: { page: dto.page, pageSize: dto.pageSize, total: 0, totalPages: 0 },
            };
        }
    ...
    async create(header: ICustomRequestHeaders, dto: CreateTransferStockDto, type: string) {
        const store_id = requireStoreId(header);
            ...
            if (getItem) {
                ...
                if (!isStockAvailable) {
                    throw new BadRequestException(
                        'Insufficient stock for item ' + getItem.name + ', remaining stock = ' + getItem.stock_quantity
                    );
                }
            } else {
